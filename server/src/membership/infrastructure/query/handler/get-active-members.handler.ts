...
export class GetActiveMembersHandler implements IQueryHandler<GetActiveMembersQuery>{
    constructor(
        @InjectRepository(MemberView) private readonly memberViewRepository: Repository<MemberView>
    ) { }

    public async execute(): Promise<MemberDto[]> {
        const members = await this.memberViewRepository.find({ where: { status: 1 } });
