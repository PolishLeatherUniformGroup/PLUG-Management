...
export class GetMemberFeesHandler
  implements IQueryHandler<GetMemberFeesQuery, MembershipFeeView[]>
{
  constructor(
    @InjectRepository(MembershipFeeView)
    private readonly memberFeeRepository: Repository<MembershipFeeView>,
  ) {}
  ...
  async execute(query: GetMemberFeesQuery): Promise<MembershipFeeView[]> {
    ...
    if (isCard) {
      fees = await this.memberFeeRepository.find({
        where: { member: { cardNumber: memberId['id'] } },
        order: { year: 'DESC' },
        relations: {
          member: false,
        },
