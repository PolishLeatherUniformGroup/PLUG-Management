...
export class GetRecommendationsHandler  implements IQueryHandler<GetRecommendationsQuery, RecommendationView[]>{
    constructor(
    ...
    async execute(query) : Promise<RecommendationView[]>{
        var id = query.id;
        const recommendations = await this.repository.find({where: {cardNumber: id['id']}, relations: {applicant: true}});
