...
export class Applicant extends AggregateRoot {
  ...
  constructor(
    ...
    super();
    (this._id = id), (this._firstName = firstName);
    this._lastName = lastName;
    this._email = email;
    this._birthDate = birthDate;
    this._phone = phone;
    this._address = address;
    this._recommendations = recommendations.map((r, i) => {
      return new Recommendation(`${id}_${i}`, r);
    });
  ...
  public endorseRecommendation(recommendationId: string) {
    this.recommendations
      ...
      ?.endorse();
    if (
      this._recommendations.every((recommendation) => recommendation.isEndorsed)
    ) {
