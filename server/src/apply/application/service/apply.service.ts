...
import { ApplicationReceived } from "src/apply/domain/events";
...
import { RecommendationView } from "src/apply/infrastructure/read-model/model/recommendation.entity";
...
export class ApplyService {
    constructor(
        ...
        @InjectRepository(RecommendationView)private readonly recommendationsRepository: Repository<RecommendationView>,
    ...
    async verifyApplication(data:EventPayloads['apply.verify-application']) {
        ...
        const recommendations = await this.recommendationsRepository
        .find({where:{applicant:{id:data.id}}});

        const cards: string[] =  recommendations.map(rec => rec.cardNumber);
        const valid = await this.verificationService.verifyCardNumbers(cards.map(card => card.toString()));
        if (valid.length == recommendations.length && valid.length == data.rcomendationsCount) {
            ...
            const command = new CancelApplicationCommand(ApplicantId.fromString(data.id));
            this.commandBus.execute(command);
        }
