...
import { ApplicantRecommendationsRequested, ApplicationReceived, ApplyEvents } from "../events";
...
it('should recreate application', () => {
    ...
    const event0= new ApplicationReceived('applicant-id', 'first-name', 'last-name', 'email', 'phone-number', new Date(), new Date(), {
        city: 'city',
        country: 'country',
        street: 'street',
        postalCode: 'zip-code',
    ...
    const event1 = new ApplicantRecommendationsRequested('applicant-id', new Date(), Money.create(100,"PLN"));
    const data1 = JSON.parse(JSON.stringify(event1));
    const deserialized0 = ApplyEvents[event0.constructor.name](...Object.values(data0));
    expect(deserialized0).toBeInstanceOf(ApplicationReceived);
    const deserialized1 = ApplyEvents[event1.constructor.name](...Object.values(data1));

    expect(deserialized0.recommendations[0].id).toBe(event0.recommendations[0].id);
...
it('should requestRecomendation', () => {
    ...
    applicant.requestRecommendations(new Date(), Money.create(100,"PLN"), ["recommender-1", "recommender-2"], ["recommender-1", "recommender-2"]);
